    ...
    if (auth.currentUser) {
        const testData = {
            ...
            questions: questions.map((question) => {
                return {
                    ...
                    answers:
                        question.answers ||
                        [...question.incorrectAnswers, question.correctAnswer].sort((a, b) =>
                            a.localeCompare(b)
                        ),
        ...
        if (authorTest) {
            setDoc(
                doc(
                    database,
                    `users/${authorTest.author}/createdTests/${testId}/usersAnswer/${auth.currentUser.uid}`
                ),
                testData
            );
        }
