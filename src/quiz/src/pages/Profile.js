...
function Profile() {
    ...
    const getAuthorTest = async () => {
        ...
        const res = await getDocs(
            collection(database, `users/${auth.currentUser.uid}/createdTests`)
        );
        const tests = [];
        res.forEach((doc) => {
            const test = doc.data();
            ...
            getDocs(
                collection(
                    database,
                    `users/${auth.currentUser.uid}/createdTests/${test.id}/usersAnswer/`
                )
            )
                .then((res) => {
    ...
    return (
        <div className='app-content'>
            ...
            <div className='profile-link-wrapper'>
                <NavLink to='/create-quiz' className='profile-link'>
                    &#9998; Create Quiz
                </NavLink>
            </div>
            <div className='profile-select'>
                <span
                    className={`profile-select__item ${
                        testShown === "complitedQuiz" ? "active" : ""
                    }`}
                    onClick={() => setTestShown("complitedQuiz")}
                >
                    Complited Quiz
                </span>
                ...
                <span
                    className={`profile-select__item ${
                        testShown === "createdQuiz" ? "active" : ""
                    }`}
                    onClick={() => setTestShown("createdQuiz")}
                >
                    Created Quiz
                </span>
            </div>
